...
public class GetProductHandler : IRequestHandler<GetProductRequest, ApiResult>
{
    ...
    public GetProductHandler(IUnitOfWork<ReadDbContext> unitOfWork, IDistributedCache cache,
    ...
    public async Task<ApiResult> Handle(GetProductRequest request, CancellationToken cancellationToken)
    {
        try
        {
            var prodcuts = await GetPostsInDatabase(request, cancellationToken);
    ...
    private async Task<Paginate<ProductViewModel>> GetPostsInDatabase(GetProductRequest request,
